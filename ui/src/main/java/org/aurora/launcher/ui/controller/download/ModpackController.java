...
public class ModpackController extends BaseResourceController {
    
    @Override
    protected CompletableFuture<List<UnifiedMod>> doSearch(String query) {
        if (unifiedSearchService != null) {
            return unifiedSearchService.search(query, null, "modpack");
        }
        return searchService.searchModpacks(query, null)
    ...
    private UnifiedMod toUnifiedMod(org.aurora.launcher.api.modrinth.ModrinthProject p) {
        ...
        mod.setPageUrl("https://modrinth.com/modpack/" + p.getSlug());
        return mod;
    }
}