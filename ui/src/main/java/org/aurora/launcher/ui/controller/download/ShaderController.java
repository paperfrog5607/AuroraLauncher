...
public class ShaderController extends BaseResourceController {
    
    @Override
    protected CompletableFuture<List<UnifiedMod>> doSearch(String query) {
        if (unifiedSearchService != null) {
            return unifiedSearchService.search(query, null, "shader");
        }
        return searchService.searchShaders(query, null)
    ...
    private UnifiedMod toUnifiedMod(org.aurora.launcher.api.modrinth.ModrinthProject p) {
        ...
        mod.setPageUrl("https://modrinth.com/shader/" + p.getSlug());
        return mod;
    }
}